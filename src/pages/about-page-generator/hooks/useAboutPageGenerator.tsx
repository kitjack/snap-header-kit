...
export const useAboutPageGenerator = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!user) {
      toast({
        ...
        description: "Please login to generate about page text.",
        variant: "destructive",
      });
      return;
    }
    ...
    if (profile && profile.credits < GENERATION_COST) {
      ...
      toast({
        ...
        description: `You need ${GENERATION_COST} credits to generate about page text.`,
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke('generate-about-page', {
        body: {
          ...formData,
          userId: user.id
        }
      ...
      if (!data || !data.aboutPageText) {
        throw new Error('Failed to generate about page text');
      }

      setResult(data.aboutPageText);
      ...
      if (profile) {
        ...
        if (updateError) {
          ...
          toast({
            title: "About page text generated!",
            description: `${GENERATION_COST} credits have been deducted from your account.`,
          });
        }
      }
    } catch (err: any) {
      console.error('Error generating about page text:', err);
      setError("Failed to generate about page text. Please try again.");
      toast({
        title: "Error",
        description: "Failed to generate about page text. Please try again.",
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
  ...
  const copyToClipboard = (text: string) => {
    ...
    toast({
      title: "Copied!",
      description: "About page text copied to clipboard.",
    });
  };
