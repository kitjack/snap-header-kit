...
export const useAcademicProjectGenerator = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke('generate-academic-topics', {
        body: {
  ...
  const renderCreditInfo = () => {
    if (!user) return <div className="text-sm text-amber-600">Login to generate project topics</div>;
